...
dataset_list = ['karate','dolphins','jazz']
for dataset in dataset_list:
    ...
    for noi in range(0, 301, 20):
        ...
        noise_lambda = torch.tensor(noise)
        for name ,para in model.named_parameters():
            ...
            model.state_dict()[name][:] += torch.rand(para.size()) * noise_lambda * torch.std(para)
...
for dataset in dataset_list:
    ...
    for noi in range(0, 16, 1):
        ...
        for name, para in model.named_parameters():
            ...
            model.state_dict()[name][:] += torch.rand(para.size()) * noise_lambda * para

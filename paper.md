...
![The hierarchical structure of the GraphSL library: in total 6 algorithms are implemented, which can be divided into two categories: prescribed methods that rely on hand-crafted rules and GNN-based methods which learn rules from graph data. \label{fig:overview}](overview.png)
